...
namespace OptimisationMethods.Core
{
    public static class Methods
    {
        ...
        private static Search Davidon(Search search, int max)
        {
            Search searchParameters = new Search(search);
            int counter = 0;
            ...
            do
            {
                expectedMin = Operations.interpolationFunctions(5, searchParameters.alphaA, searchParameters.alphaB, 0,
                    searchParameters.CurrentPoint, searchParameters.SearchDirection);

                if (Operations.Derivative(searchParameters.CurrentPoint + expectedMin * searchParameters.SearchDirection,
                        searchParameters.SearchDirection) < 0)
                {
                    searchParameters.alphaA = expectedMin;
                }
                else
            ...
            } while (Math.Abs(Operations.Derivative(
                         searchParameters.CurrentPoint + expectedMin * searchParameters.SearchDirection,
                         searchParameters.SearchDirection)) > searchParameters.DiscretizationError && counter < max);
        ...
        private static Search ComboSearch(Search search)
        {
            ...
            Search minimumSearchParameters = Dsc(reduceIntervalSearchParameters, maxIterations);
